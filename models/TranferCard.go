...
func (item *TranferCard) FindList(database *gorm.DB, page Page, playerName string) ([]map[string]interface{}, int64, error) {
	...
	db = db.Joins("LEFT JOIN (select * from customer_users) cso on tranfer_cards.owner_uid_old = cso.uid")

	db = db.Joins("LEFT JOIN customer_users on tranfer_cards.owner_uid = customer_users.uid")

	if item.PartnerUid != "" {
		db = db.Where("tranfer_cards.partner_uid = ?", item.PartnerUid)
	}
	if item.CourseUid != "" {
		db = db.Where("tranfer_cards.course_uid = ?", item.CourseUid)
	}
	if item.OwnerUid != "" {
