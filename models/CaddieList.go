...
func addFilter(db *gorm.DB, item *CaddieList) *gorm.DB {
	...
	if item.CaddieName != "" || item.CaddieCode != "" {
		db = db.Where("name LIKE ?", "%"+item.CaddieName+"%").Or("code = ?", item.CaddieCode)
	}
...
func (item *CaddieList) FindList(database *gorm.DB, page Page) ([]Caddie, int64, error) {
	...
	if total > 0 && int64(page.Offset()) < total {
		if item.Month != "" {
			...
			db = page.Setup(db).Preload("CaddieCalendar", "DATE_FORMAT(apply_date, '%Y-%m') = ?", time.Now().Format("2006-01")).Preload("GroupInfo").Debug().Find(&list)
		}
	}
