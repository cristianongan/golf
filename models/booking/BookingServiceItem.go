...
func (item *BookingServiceItem) FindReport(database *gorm.DB, page models.Page, fromDate, toDate, typeService string) ([]map[string]interface{}, int64, error) {
	...
	if typeService == "KIOSK" {
		if typeService == "KIOSK" {
			db = db.Select(`tb2.booking_date, SUM(if(tb1.type = ?, tb1.amount, 0)) AS total_kiosk`, constants.KIOSK_SETTING)
		}
	}

	if typeService == "MINI_B" {
		if typeService == "MINI_B" {
			db = db.Select(`tb2.booking_date, SUM(if(tb1.type = ?, tb1.amount, 0)) AS total_bar`, constants.MINI_B_SETTING)
		}
	}
