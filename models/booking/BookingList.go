...
func (item *BookingList) FindBookingListWithSelect(database *gorm.DB, page models.Page, isGroupBillCode bool) (*gorm.DB, int64, error) {
	...
	db = db.Preload("MemberCard").Joins("LEFT JOIN member_cards as tb1 ON bookings.member_card_uid = tb1.uid")
	db = db.Preload("MemberCardOfGuest").Joins("LEFT JOIN member_cards as tb2 ON bookings.member_uid_of_guest = tb2.uid")
...
func (item *BookingList) FindListBookingWithBuggy(database *gorm.DB, page models.Page) ([]ResBookingWithBuggyFeeInfo, int64, error) {
	...
	db = db.Select("bookings.booking_date, JSON_VALUE(bookings.buggy_info,'$.code') as buggy_code, booking_service_items.name as buggy_type, bookings.bag, bookings.tee_off_time as tee_off, bookings.customer_name, bookings.guest_style, bookings.card_id, JSON_VALUE(bookings.agency_info,'$.name') as agency_name, booking_service_items.hole, bookings.caddie_id, booking_service_items.amount as fee")
...
func (item *BookingList) ReportAllBooking(database *gorm.DB) (ReportBooking, error) {
	...
	db = db.Select(`SUM(bag_status = 'TIMEOUT' AND added_round = 0 AND moved_flight = 0) AS time_out,
					...
					SUM(check_in_time > 0 AND customer_type <> 'NONE_GOLF' AND added_round = 0 AND moved_flight = 0) AS check_in,
					SUM(check_out_time > 0 AND customer_type <> 'NONE_GOLF' AND added_round = 0 AND moved_flight = 0) AS check_out,
